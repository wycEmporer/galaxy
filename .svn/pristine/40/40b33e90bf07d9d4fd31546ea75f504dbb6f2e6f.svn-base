    ...
    <div class="confirm-info-detail-box2">
      <div v-for="(item, key) in trip.triplist" :key="key">
        ...
        <template v-for="(voyage, index) in item.voyageinfo">
          ...
          <div class="voyage_duration_time text-style marBot30" v-if="item.voyageinfo.length > 1 && index != item.voyageinfo.length -1" :key="voyage.status">1 Hour 30 Minute</div>
        </template>
